...
import { Check, Copy, FileText, Minimize2, Trash2 } from "lucide-react"
...
export function HtmlFormatter() {
  ...
  const formatHtml = () => {
    ...
    setTimeout(() => {
      try {
        const formatted = formatHtmlString(input)
        setOutput(formatted)
        setError("")
      } catch (e) {
        setError("处理失败")
    ...
    <div className="grid gap-6">
      <Card className="border-primary/20 transition-all duration-300 hover:border-primary/30">
        ...
        <CardContent className="space-y-4">
          <Textarea
            placeholder="<div><h1>标题</h1><p>内容</p></div>"
          ...
          <div className="flex gap-3">
            ...
            >
              <FileText className={`w-4 h-4 ${isProcessing ? "animate-spin" : ""}`} />
      ...
      {(output || error) && (
        <Card className="border-primary/20 animate-in transition-all duration-300 hover:border-primary/30">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="font-mono text-lg">{error ? "$ error" : "$ output.html"}</CardTitle>
